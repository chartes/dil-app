"""
Modèle V1.1.2 de la base de données de DIL-App

Commentaires:
 - On part du modèle V1.1.1
 - Modèle très élaboré et standardisé
 - On standardise un peu plus adresse :
    - Table intermediaire pour les adresses - Imprimeur a une date et avec un type donnée
    - Table pour répertorier les adresses
    - On lie les adresses à la table ville
        - suppose de disposer de cette information - a voir avec chercheuse si working_city == ville de l'adresse est suffisant)
        - suppose de réussir a séparer la date de l'adresse
- On utilise un lien IIIF pour les images si dispo (suppose de faire un travail d'analyse sur les images provenant de bnf)
"""
...
class Adress(Base):
    ...
    street = Column(String(255), nullable=False, unique=False)
    city_id = Column(Integer, ForeignKey("cities.id"), nullable=True, unique=False)
    city = relationship("City", back_populates="adresses")

class PrinterAdress(Base):
    """Table intermediaire pour les adresses des imprimeurs
    """
    __tablename__ = "printer_adress"
    __prefix__ = "printer_adress"

    id = Column(Integer, primary_key=True, autoincrement=True, nullable=False, unique=True)
    printer_id = Column(Integer, ForeignKey("printers.id"), nullable=False, unique=False)
    adress_id = Column(Integer, ForeignKey("adresses.id"), nullable=False, unique=False)

    type = Column(Enum(AdressTypeLabels), nullable=False, unique=False)
    date_occupation = Column(String(25), nullable=True, unique=False)

    adress = relationship("Adress", back_populates="printer")
...
class Image(Base):
    ...
    # manifest JSON?
    iiif_url = Column(String(255), nullable=True, unique=False)

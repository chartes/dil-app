...
def load_tsv_to_db(file_path, table_model):
    try:
        with get_session() as session:
            print(
                f"==> Loading data from {file_path} to {table_model.__tablename__} table."
            )
            ...
            data["_id_dil"] = data["_id_dil"].apply(
                lambda x: (
                    x
                    if x
                    else generate_random_uuid(
                        prefix=table_model.__prefix__, provider="dil"
                    )
                )
            )
            ...
            print(
                f"==> {len(records)} rows inserted in {table_model.__tablename__} table."
            )
        ...
        print(
            f"Erreur lors de l'importation dans la table {table_model.__tablename__}: {e}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create the database and import data from TSV files."
    )
    parser.add_argument(
        "--db",
        help="Database path and name",
        default=f"{__prefix_model__}_{__version__}.db",
    )

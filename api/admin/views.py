...
class PopupCreateMixin:
    """Mixin to handle popup creation of related models in the admin interface."""

    @expose("/new/", methods=("GET", "POST"))
    def create_view(self):
        original_template = self.create_template

        if request.args.get("popup") == "1":
            self.create_template = "admin/popup_create.html"
    ...
    def login(self):
        ...
        if form.validate_on_submit():
            ...
            if user and user.check_password(form.password.data):
                ...
                flash(
                    f"Vous êtes connecté en tant que {current_user.username} !",
                    "success",
                )
                return redirect(url_for("admin.index"))
            else:
...
class GlobalModelView(ModelView):
    ...
    # Optional flags to force permissions for edit, delete,
    # create and view details actions.
    force_can_edit = None
    force_can_delete = None
    force_can_create = None
    force_can_view_details = None

    def is_accessible(self):
        """Check if the user has access to the view."""
        if current_user.is_authenticated:
            default_can_edit = current_user.role in can_edit_roles
            default_can_delete = current_user.role in can_delete_roles
            default_can_create = current_user.role in can_create_roles
        else:
        ...
        self.can_delete = (
            ...
            if self.force_can_delete is not None
            else default_can_delete
        )
        self.can_create = (
            self.force_can_create
            if self.force_can_create is not None
            else default_can_create
        )
        self.can_view_details = (
            self.force_can_view_details
            if self.force_can_view_details is not None
            else True
        )
...
class UserView(ModelView):
    ...
    column_list = ["id", "username", "email", "role", "created_at", "updated_at"]
    column_labels = {
        "id": "ID",
    ...
    form_columns = ["username", "email", "role", "new_password"]

    form_extra_fields = {
        "new_password": PasswordField("Nouveau mot de passe", id="new_password_field"),
    }
        ...
        if form.new_password.data:
            if model.id == 1:
                if current_user.id != 1:
                    flash(
                        "Vous ne pouvez pas modifier le mot de passe de ce compte administrateur.",
                        "danger",
                    )
                else:
    ...
    def delete_model(self, model):
        ...
        if model.role == "Admin":
            if current_user.role != "ADMIN":
                flash(
                    "Vous ne pouvez pas supprimer un compte administrateur.", "danger"
                )
                return False
...
class PrinterView(GlobalModelView):
    ...
    column_formatters_detail = {
        "personal_information": lambda v, c, m, p: (
            Markup(m.personal_information)
            if m.personal_information
            else "Aucune information personnelle disponible."
        ),
        "professional_information": lambda v, c, m, p: (
            Markup(m.professional_information)
            if m.professional_information
            else "Aucune information professionnelle disponible."
        ),
    }
    ...
    column_filters = ["lastname", "firstnames", "birth_city_label"]
    ...
    inline_models = [
        (
            PersonHasAddresses,
            {
                "form_columns": ["id", "address_persons", "date_occupation"],
                "column_labels": {
                ...
                "form_args": {
                    "address_persons": {
                        ...
                        "description": _format_link_add_model(
                            ...
                            href=f"{BASE_PREFIX_ADD_MODEL}address/new/",
                            target_field="address_persons",
                            modal_title="Ajouter une nouvelle adresse au référentiel",
                        ),
                    },
                    "date_occupation": {
        ...
        (
            Patent,
            {
                ...
                "form_widget_args": {
                    ...
                    "addresses_container": {
                        "onchange": "makeAddress(this)",
                        "class": "addresses-container-attach",
                    },
                    "relation_container": {
                        "onchange": "makeRelation(this)",
                        "class": "relation-container-attach",
                    },
                },
                "form_ajax_refs": {
                ...
                "form_args": {
                    ...
                    "images": {
                        "label": _format_label_form_with_tooltip(
                            label="Images", comment="Images liées au brevet."
                        ),
                        "description": _format_link_add_model(
                            description="une nouvelle image",
                            href=f"{BASE_PREFIX_ADD_MODEL}image/new/",
                            target_field="images",
                            modal_title="Ajouter une nouvelle image au référentiel",
                        ),
                    },
                },
            },
    ...
    form_args = {
        "lastname": {
            "label": _format_label_form_with_tooltip(
                label="Nom", comment="Nom de l'imprimeur."
            )
        },
        "city": {
            "label": _format_label_form_with_tooltip(
    ...
    def on_model_change(self, form: object, model: object, is_created: bool) -> object:
        ...
        handler = PrinterModelChangeHandler(
            session=self.session, model=model, form=form
        )
    ...
    def render(self, template, **kwargs):
        ...
        if template == "admin/details.printer.html":
            ...
            id_patents = [
                evt.id
                for evt in get_printer(session, {"id": kwargs["model"].id}).patents
            ]
            if len(patents) == len(id_patents):
                return super(PrinterView, self).render(
                    template,
                    **kwargs,
                    patents=list(zip(id_patents, patents)),
                    printer=printer,
                )
            else:
    ...
    def get_list(self, page, sort_field, sort_desc, search, filters, page_size=None):
        ...
        if sort_field:
            ...
            filtered_rows.sort(
                key=lambda x: (
                    getattr(x, sort_field, "").lower() if getattr(x, sort_field) else ""
                ),
                reverse=reverse,
            )
    ...
    def get_image_details(self):
        ...
        if request.method in ["GET", "POST"]:
            image_id = request.args.get(
                "id"
            )  # Retrieve the image ID from the GET parameters
            patent_id = request.args.get(
                "patent_id"
            )  # Retrieve the patent ID from the GET parameters
            if not image_id:
                return jsonify({"error": "This patent has no image."}), 200
            image = session.query(Image).filter_by(id=image_id).first()
            try:
                is_pinned = (
                    session.query(PatentHasImages)
                    .filter_by(image_id=image_id, patent_id=patent_id)
                    .first()
                    .is_pinned
                )
            ...
            return jsonify(
                {
                    ...
                    "label": image.label,
                    "img_name": image.img_name,
                    "img_url": url_for("static", filename=f"images_store/{img_name}")
                    if (img_name != "unknown.jpg") or (image.iiif_url is not None)
                    else url_for("static", filename="icons/preview-na.png"),
                    "img_iiif_url": image.iiif_url,
                    "is_pinned": is_pinned,
                    "fallback_url": url_for("static", filename="icons/preview-na.png"),
                    "fallback_iiif_url": url_for(
                        "static", filename="icons/preview-na-iiif.png"
                    ),
                }
            )
    ...
    def ajax_addresses(self):
        ...
        search = request.args.get("q", "")  # Retrieve the search term from parameters
        query = (
            session.query(Address).filter(Address.label.ilike(f"%{search.lower()}%"))
        ).limit(20)  # limit to n results to avoid performance issues
    ...
    def ajax_printer(self, person_id):
        ...
        person = get_printer(session, {"id": person_id})
        return jsonify({"id": person_id, "text": repr(person)})

    @expose("/get_address/<int:address_id>", methods=["GET"])
    def ajax_address(self, address_id):
        """Retrieves details of a printer from the database."""
        address = get_address(session, {"id": address_id})
        return jsonify({"id": address_id, "text": repr(address)})
    ...
    def get_patent_relations(self, person_id):
        ...
        for pid in patents_id:
            relations = (
                ...
                .filter(
                    PatentHasRelations.person_id == person_id,
                    PatentHasRelations.patent_id == pid,
                )
                .all()
            )
            ...
            for relation in relations:
                relation_type = [
                    k for k, v in type_patent_relations.items() if v == relation.type
                ][0]
                grouped_relations[pid].append(
                    {"id": relation.person_related_id, "data": relation_type}
                )
        ...
        for pid in patents_id:
            ...
            for address in addresses:
                grouped_addresses[pid].append(
                    {"id": address.address_id, "data": address.date_occupation}
                )

        return jsonify(grouped_addresses)


class ImageView(PopupCreateMixin, GlobalModelView):
    force_can_edit = False
    force_can_delete = True
    force_can_create = True
    force_can_view_details = True
    ...
    column_formatters = {
        "img_name": lambda v, c, m, p: Markup(
            ...
            if m.img_name != "unknown.jpg"
            else (
                f'<img src="{m.iiif_url}" style="max-width: 200px; max-height: 200px;">'
                if m.iiif_url
                else f'<img src="{url_for("static", filename="icons/preview-na.png")}" style="max-width: 200px; max-height: 200px;">'
            )
        ),
        "reference_url": lambda v, c, m, p: (
            _format_href(prefix_url="", label=m.reference_url)
            if m.reference_url and m.reference_url != "unknown_url"
            else "Inconnue"
        ),
        "iiif_url": lambda v, c, m, p: (
            _format_href(prefix_url="", label=m.iiif_url) if m.iiif_url else "Inconnue"
        ),
    }
    ...
    form_columns = [
        ...
        "img_name",
        "iiif_preview",
        # Don't use iiif_url directly in the form,
        # it's generated automatically from the
        # reference_url if the source is Gallica.
        # Cf. utils_gallica.py
    ]
    ...
    form_extra_fields = {
        "is_gallica_source": SelectField(
            "Image disponible sur Gallica-BnF (IIIF) ?",
            choices=[("no", "Non"), ("yes", "Oui")],
            default="yes",
            render_kw={"id": "is_gallica_source"},
        ),
        "img_name": ImageUploadField(
            "Charger une image locale",
            base_path=settings.IMAGE_STORE,
        ...
        "iiif_preview": StringField(
            "URL IIIF générée automatiquement",
            render_kw={"readonly": True, "id": "iiif_preview"},
        ),
    }
    ...
    form_args = {
        "label": {
            ...
            "description": (
                "Champ obligatoire. "
                "Exemple : <b>Vue de la ville de Paris</b> ; "
                "<b>Plan de la ville de Paris (1846)</b>."
            ),
        },
        "is_gallica_source": {
            "label": "Image disponible sur Gallica-BnF ?",
            "description": (
                "Cocher si l'image provient de Gallica. "
                "Dans ce cas, renseigner l'URL Gallica et ne pas charger de fichier local."
            ),
        },
        "reference_url": {
            ...
            "description": (
                "URL de référence de l'image. "
                "Il peut s'agir : d'une URL Gallica ou de toute autre URL associé à l'image déposée. "
                "Exemples : "
                "<b>https://gallica.bnf.fr/ark:/12148/btv1b55002798b</b> "
                "ou "
                "<b>https://archives.creuse.fr/...</b>"
            ),
        },
        "img_name": {
            "label": "Image locale",
    ...
    def create_form(self, obj=None):
        ...
        if hasattr(form, "is_gallica_source"):
            form.is_gallica_source.data = "no"

        if hasattr(form, "iiif_preview"):
            form.iiif_preview.data = ""

        return form

    def edit_form(self, obj=None):
        """Pre-forms the edit form with existing data, including
        custom handling for Gallica source and IIIF preview."""
        form = super().edit_form(obj)
        if obj:
            if hasattr(form, "is_gallica_source"):
                form.is_gallica_source.data = "yes" if obj.iiif_url else "no"

            if hasattr(form, "iiif_preview"):
                form.iiif_preview.data = obj.iiif_url or ""

        return form

    def on_model_change(self, form, model, is_created):
        """Business logic to handle the
        image source (Gallica or local) and associated
        validations when creating or editing an image
        ...
        if is_gallica:
            if not reference_url:
                raise ValidationError(
                    "L'URL Gallica est obligatoire si l'image provient de Gallica-BnF."
                )

            if not is_gallica_url(reference_url):
                raise ValidationError(
                    "L'URL de référence doit être une URL Gallica valide."
                )

            try:
                iiif_url = gallica_url_to_iiif(reference_url)
            except ValueError as e:
                raise ValidationError(str(e))

            if uploaded_file:
                raise ValidationError(
                    "Ne chargez pas de fichier local si l'image provient de Gallica."
                )

            model.reference_url = reference_url
            model.iiif_url = iiif_url
            ...
            # Keep the existing reference_url if it exists,
            # otherwise set to "unknown_url"
            model.reference_url = reference_url if has_reference_url else "unknown_url"
            model.iiif_url = None

            if uploaded_file is None and not model.img_name:
                model.img_name = "unknown.jpg"
...
class PatentHasRelationsView(GlobalModelView):
    ...
    column_formatters = {
        ...
        "type": lambda v, c, m, p: (
            Markup(f"<span>{m.type}</span>") if m.type else "(Aucun type)"
        ),
    }


class AddressView(PopupCreateMixin, GlobalModelView):
    ...
    column_formatters = {
        "city_id": lambda v, c, m, p: (
            Markup(
                f'<a href="{url_for("city.details_view", id=m.city_id)}">{m.city}</a>'
            )
            if m.city_label
            else "(Aucune ville)"
        )
    ...
    form_columns = ["not_french_address", "label", "city_label", "city"]
    ...
    # add checkbox to select the type of address
    # (french by default)
    form_extra_fields = {
        "not_french_address": BooleanField(
            label="Adresse hors France ?", render_kw={"id": "not_french_address"}
        )
    }
...
class CityView(PopupCreateMixin, GlobalModelView):
    ...
    column_formatters = {
        ...
        ''')
            if m.long_lat
            else "(Pas de coordonnées disponibles)"
        ),
        ...
        "geoname_id": lambda v, c, m, p: _format_href(
            label=m.geoname_id, prefix_url="https://www.geonames.org/"
        ),
        "wikidata_item_id": lambda v, c, m, p: _format_href(
            label=m.wikidata_item_id, prefix_url="https://www.wikidata.org/wiki/"
        ),
        "dicotopo_item_id": lambda v, c, m, p: _format_href(
            label=m.dicotopo_item_id, prefix_url="https://dicotopo.cths.fr/places/"
        ),
        "databnf_ark": lambda v, c, m, p: _format_href(
            label=m.databnf_ark, prefix_url="https://data.bnf.fr/fr/"
        ),
        "viaf_id": lambda v, c, m, p: _format_href(
            label=m.viaf_id, prefix_url="https://viaf.org/fr/viaf/"
        ),
        "siaf_id": lambda v, c, m, p: _format_href(
            label=m.siaf_id, prefix_url="https://francearchives.gouv.fr/fr/location/"
        ),
    }

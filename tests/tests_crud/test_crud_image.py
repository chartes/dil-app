import pytest
from api.models.models import Person, Patent, Image, PatentHasImages
...
def test_image_constraints():
    ...
    with local_session as session:
        ...
        patent_image1 = PatentHasImages(
            patent_id=patent.id, image_id=image1.id, is_pinned=True
        )
        ...
        assert (
            ...
            .filter(PatentHasImages.patent_id == patent.id)
            .one()
            is not None
        )
        ...
        patent_image1_id = (
            session.query(PatentHasImages)
            .filter(PatentHasImages.image_id == image1.id)
            .one()
            .id
        )
        assert (
            session.query(PatentHasImages)
            .filter(PatentHasImages.id == patent_image1_id)
            .one()
            .is_pinned
            is True
        )

        patent_image2 = PatentHasImages(
            patent_id=patent.id, image_id=image2.id, is_pinned=True
        )
        session.add(patent_image2)
        session.commit()
        patent_image2_id = (
            session.query(PatentHasImages)
            .filter(PatentHasImages.image_id == image2.id)
            .one()
            .id
        )
        ...
        assert (
            session.query(PatentHasImages)
            .filter(PatentHasImages.id == patent_image2_id)
            .one()
            .is_pinned
            is True
        )

        patent_image3 = PatentHasImages(
            patent_id=patent.id, image_id=image3.id, is_pinned=True
        )
        ...
        patent_image3_id = (
            session.query(PatentHasImages)
            .filter(PatentHasImages.image_id == image3.id)
            .one()
            .id
        )
        assert (
        ...
        assert (
            session.query(PatentHasImages)
            .filter(PatentHasImages.id == patent_image3_id)
            .one()
            .is_pinned
            is True
        )
